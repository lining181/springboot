...
public class MailSender {
     ...
     * @throws Exception
     */
    public boolean sendTextMail(MailInfoEntity mailInfo, int mailType) throws Exception {
     ...
     * @throws Exception
     */
    public boolean sendHtmlMail(MailInfoEntity mailInfo, int mailType) throws Exception {
     ...
     */
    private Address[][] getMailToAddress(MailInfoEntity mailInfo, int mailType) throws AddressException {
        Address[] toAdds = null;
        Address[] ccAdds = null;
        ...
        if (null != mailInfo.getNotifyCc() && !"".equals(mailInfo.getNotifyCc())){
     ...
     * @return
     */
    public MailInfoEntity getMailInfo() {
        MailInfoEntity info = new MailInfoEntity();
        info.setMailHost("邮件提供商的stmp服务器");
        info.setMailPort("465");
        info.setUsername("邮件发送人邮箱");
        info.setPassword("发送人邮箱密码");
        info.setNotifyTo("邮件接收人");
        info.setNotifyCc("邮件抄送人");
        info.setSubject("java send mail test");
        info.setContent("中文编码测试，看是不是乱码");
        info.setAttachFileNames(new String[]{"附件一", "附件二"});//添加附件
        return info;
    }
}
